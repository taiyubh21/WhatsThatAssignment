...
import {
  ...
  FlatList, View, ActivityIndicator, Text, TextInput, ScrollView, TouchableOpacity, Modal, StyleSheet,
} from 'react-native';
...
class ConversationDisplay extends Component {
  ...
  async newMessage() {
    return fetch(
      `http://localhost:3333/api/1.0.0/chat/${this.state.currentChatId}/message`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
  ...
  async updateMessage(messageID) {
    ...
    if (this.state.sendMessage !== '') {
      toSend.message = this.state.sendMessage;
    }

    console.log(JSON.stringify(toSend));
    return fetch(
      `http://localhost:3333/api/1.0.0/chat/${this.state.currentChatId}/message/${messageID}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.messageContainer}>
          <TextInput style={styles.textStyle} placeholder="new message..." onChangeText={(message) => this.setState({ message })} value={this.state.message} />
          <TouchableOpacity
        ...
        >
          <View style={styles.modalContainer}>
            ...
            <View style={styles.modalStyle}>
              <TextInput style={styles.textStyle} placeholder="Message..." onChangeText={(sendMessage) => this.setState({ sendMessage })} defaultValue={this.state.selectedMessage.message} />
              <Text>{' '}</Text>
              ...
              <View style={styles.buttonContainer}>
                <TouchableOpacity
                  ...
                  onPress={() => { this.onPressButton(this.state.selectedMessage.message_id); }}
                >
                  <Text style={{ color: 'white', fontWeight: 'bold' }}>Update</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={styles.deleteBtn}
                  onPress={() => { this.deleteMessage(this.state.selectedMessage.message_id); }}
                >

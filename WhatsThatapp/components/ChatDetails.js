...
import {
  ...
  Text, TextInput, View, TouchableOpacity, ActivityIndicator, ScrollView, FlatList, Image, StyleSheet,
} from 'react-native';
...
class ChatDetails extends Component {
      ...
      .then((responseJson) => {
        this.setState({
          ...
          if (this.state.chatData.members && this.state.chatData.members.length > 0) {
            for (let i = 0; i < this.state.chatData.members.length; i += 1) {
  ...
  async updateChat() {
    ...
    if (this.state.chatname !== '') {
      toSend.name = this.state.chatname;
    }

    console.log(JSON.stringify(toSend));
    return fetch(
      `http://localhost:3333/api/1.0.0/chat/${this.state.currentChatId}`,
      {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
  ...
  async deleteMember(chatuserID) {
    return fetch(
      `http://localhost:3333/api/1.0.0/chat/${this.state.currentChatId}/user/${chatuserID}`,
      {
        method: 'DELETE',
        headers: {
          'X-Authorization': await AsyncStorage.getItem('whatsthat_session_token'),
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.form}>
          <TextInput style={styles.textInput} placeholder="Chat name..." onChangeText={(chatname) => this.setState({ chatname })} defaultValue={this.state.chatData.name} />
